...
def find_vendors():
    """
    The URL is hardcoded to the Photography vendors in San Antonio search
    results.

    TODO: This function can easily be rewritten to grab the vendor information
    for each category and return a namedtuple that contains all relevant info
    as well as the city (and state, don't forget the state) and vendory category

    @param: None - but need to add some
...
# This happens to just scrape the photography vendors bc
# that is where the URL is set to.
photo_vendors = find_vendors()
